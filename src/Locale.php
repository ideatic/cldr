...
abstract class i18n_Locale
{
    ...
    public static function factory($locale): self
    {
        ...
        throw new Exception_InvalidArgument("The culture '{$locale[0]}' does not exist or is invalid", $locale);
    }

    /**
     * Obtiene una lista de todos los locales disponibles
     *
     * @param string $name Si se indica, obtiene un valor que indica si existe ese locale
     *
     * @return string[]|bool
     */
    public static function available(?string $name = null)
    {
        $path = SYS_PATH . '/libraries/i18n/Locale';

        if (isset($name)) {
            return IO::exists("{$path}/{$name}.php");
        } else {
            $found = [];
            $files = IO::get($path, 0);
            foreach ($files as $file) {
                $name = IO::getFilenameWithoutExtension($file);
                $found[] = $name;
            }
            return $found;
        }
